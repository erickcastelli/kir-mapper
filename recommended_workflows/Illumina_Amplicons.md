kir-mapper workflow for Illumina sequencing from Amplicons
=======
...
The amplicon must include the entire gene segment, or at least all exons.
...
Open the file `/output_folder/ncopy/thresholds.txt`. These are the thresholds used. Each value is separated by a ":".

Open the .html files in `/output_folder/ncopy/plots` in a browser. Define the thresholds to separate samples into only two groups, as follows:

- samples with no copy of the target gene (ratio around zero), that must be under the red line;
- samples with any copy of the target gene (1, 2, or more), which must be between the blue and orange line. 

The rationale is that the amplicons are not suitable for read-depth comparisons, so it is not possible to define copy numbers, except for the total absence of the gene.
...
Because copy numbers cannot be defined, genotyping accuracy may be reduced in samples with three or more copies of a specific target. In addition, you cannot differentiate between an individual presenting two copies of the same allele or just one copy of that allele.