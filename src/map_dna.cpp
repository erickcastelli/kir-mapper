...
void main_dna_map ()
{
    ...
    if (((v_db == "") || (v_check == 0)) || (v_sample == ""))
    {
        ...
        screen_message (screen_size, 2, "-error       threshold for nucleotide quality trimming [" + to_string(v_mtrim_error).substr(0,4) + "]", 1, v_quiet);
        screen_message (screen_size, 2, "-tolerance   fraction of mismatches allowed [" + to_string(v_tolerance).substr(0,4) + ", from 0.01 to 0.10]", 1, v_quiet);
        screen_message (screen_size, 2, "-downsample  downsampling for the adjustment [" + to_string(downsampling) + "]", 1, v_quiet);
    ...
    {
        warnings.push_back ("You must indicate r1 and r2 when in paired-end mode.");
        v_sample = "";
        v_r1 = "";
        main_dna_map();
        return;
    ...
    if (v_rna == 0) {
        screen_message (screen_size, 0, "Detecting locus presence ...", 2, v_quiet);
        ...
        screen_message (screen_size, 0, "Detecting locus presence: done", 1, v_quiet);
        
        if (locus_depth["KIR3DL3"] < 5)
        {
            v_message = "Very low coverage for the reference. Quitting";
            warnings.push_back (v_message);
            return;
        }
...
//            if (data[0] == "A00296:35:HCF5HDSXX:4:1532:8214:12179") {cout << line << endl;}
    ...
    v_log = v_output + "/log/adjusted_sort.log";
    ...
    v_log = v_output + "/log/unique_sort.log";
    ...
    v_bam_nodup = v_output + v_sample_sub + ".unique.nodup.bam";;
    v_bam_nodup_metrics = v_output + v_sample_sub + ".unique.nodup.metrics";
    
    v_log = v_output + "/log/unique_nodup.log";

    found = v_picard.find(".jar");
    if (found!=std::string::npos)
    {
        v_command = "java -jar " + v_picard + " MarkDuplicates I=" +  v_bam_out + " O=" + v_bam_nodup + " M=" + v_bam_nodup_metrics + " VALIDATION_STRINGENCY=SILENT 2>" + v_log;
    }
