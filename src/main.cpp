...
string Program_version = "0.6.5";
string Program_author = "Erick C. Castelli";
string Program_date = "April 12th 2024";
string Program_website = "www.castelli-lab.net/apps/kir-mapper";
...
void main_help(void)
{
 ...
 //   screen_message (screen_size, 0, "           exomebias     test exomes for capture bias", 1, v_quiet);
    ...
    if (v_useconfig == 0) {
        ...
        if (config)
        {
            for( std::string line; getline( config, line ); )
            {
                ...
                if (v_set[0] == "picard") {v_picard = v_set[1];}
...
int main(int argc, const char * argv[]) {
    ...
    if (! fileExists(v_bwa)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0; }

    if (! fileExists(v_samtools)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}

    if (! fileExists(v_bcftools)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}
    
    if (! fileExists(v_db)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}

    if (! fileExists(v_star)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}

    if (v_whats != "DISABLED") {
        if (! fileExists(v_whats)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}
    }
  
    if (v_picard != "DISABLED") {
        if (! fileExists(v_picard)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}
    }
    
    if (! fileExists(v_freebayes)) {cout << endl << "You need to run 'kir-mapper setup' again." << endl << endl; main_setup(); return 0;}
