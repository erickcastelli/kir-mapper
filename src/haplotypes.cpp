...
//        screen_message (screen_size, 2, "--short         do not include 2DP1 and 3DP1", 1, v_quiet); 
        ...
        screen_message (screen_size, 2, "--force         bypass the minimum number of samples", 1, v_quiet); 
	...
	debug_message("Starting fake position definition ...");
	...
	if ((! fileExists(v_output)) || (! fileExists(v_output + "/genotype")))
    {
        v_message = "You must indicate a valid output folder, with 'map' and 'ncopy'";
        warnings.push_back (v_message);
        return;
	...
	debug_message("Output structure ...");
	...
	debug_message("Checking genotypes ...");
	...
	debug_message("Loading copy numbers ...");
	...
	map <pair<string,string>,string> calldata;
	map <pair<string,string>,string> callratio;
	map <pair<string,string>,string> callmiss;
    ...
    for (auto gene : fake_gene_positions)
    {
		...
		debug_message("Loading calls for " + gene.first + " ...");
		string callfile = v_source_genotype + "/" + gene.first + "/calls/" + gene.first + ".calls.txt";
		ifstream call (callfile.c_str());
        for( std::string line; getline( call, line ); )
        {
            if (line == "") {continue;}
			vector <string> data;
			boost::split(data,line,boost::is_any_of("\t"));
			if (data[1] == "Copy_number") {continue;}
			pair <string,string> k;
			k = make_pair(data[0], gene.first);
			calldata[k] = data[2];
			callratio[k] = data[3];
			callmiss[k] = data[4];
        }
		...
		debug_message("Loading VCF for " + gene.first + " ...");
	...
	if (v_haploforce == 1) {
		if (sample_list.size() < 20)
		{
			cout << "The minimum sample size required by shapeit4 is 20. Quitting..." << endl;
			return;    
		}
	}
	...
	debug_message("Writing fake diploid VCF ... start");
    ...
    fakevcf.close();
	debug_message("Writing fake diploid VCF ... end");



    debug_message("Normalizing VCF ... start");
	string outnorm = master_output + "/fake_diploid_for_shapeit4.bi.vcf";
    v_command = v_bcftools + " norm -m-any -o " + outnorm + " " + file;
    out = GetStdoutFromCommand(v_command);
    debug_message("Normalizing VCF ... end");

    debug_message("Compressing VCF ... start");
    v_command = "bgzip -f " + outnorm;
    out = GetStdoutFromCommand(v_command);
	debug_message("Compressing VCF ... end");

    debug_message("Indexing VCF ... start");
    v_command = "tabix -p vcf " + outnorm + ".gz";
    out = GetStdoutFromCommand(v_command);
    debug_message("Indexing VCF ... end");
                    ...
                    poolshapeit.enqueue([item]
                        {
							...
							haplo_mtx.lock();
							debug_message("Command end: " + item);
							haplo_mtx.unlock();
                            return 1;
                        })
                );
    ...
    debug_message("Comparing haplotypes ... start");
    ...
    if (v_recall == 0) {
        ...
        for (int a = 1; a <= v_replicates; a++)
        {
            ...
            string outshapeit = shapeit_dir + "/shapeit_run" + to_string(a) + ".vcf";
			if (! fileExists(outshapeit )) {cout << "Something went wrong!" << endl << endl; return;}
		...
		debug_message("Comparing haplotypes ... end");
		
		debug_message("Writing best haplotypes ... start");
        ...
        bestvcf.close();
		debug_message("Writing best haplotypes ... end");
		...
		debug_message("Writing final haplotypes ... start");
		...
		debug_message("Writing final haplotypes ... end");
