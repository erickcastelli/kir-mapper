...
void main_genotype() {
    ...
    if (((v_db == "") || (v_check == 0)))
    {
        ...
        v_message = "Usage:     kir-mapper genotype -output map_output_folder <options>";
        screen_message (screen_size, 0, v_message, 1, v_quiet);
        screen_message (screen_size, 0, "", 1, v_quiet);
        ...
        screen_message (screen_size, 2, "--two_copies    force two copies when gene is present", 1, v_quiet);
   ...
   debug_message("Checking database - done");
    ...
    debug_message("Checking hg39 - start");

    v_ref = v_db + "/reference/hg38/hg38DH.fa";
    if (! fileExists(v_ref))
    {
...
//            if (copynumber[k] == "3") {continue;}
			...
			if (copynumber[k] == "2")
			{
                ...
                if (! fileExists(vcfoutwhats)) { phasing_cmds.push_back(cmd);}
			}
...
//            string sample = item.first;
            ...
            if (fileExists(check_out_file)) {done++;return 1;}
