...
void main_setup() {
    ...
    v_message = "kir-mapper needs some third-party software to work properly.";
    screen_message (screen_size, 0, v_message, 1, v_quiet);
    screen_message (screen_size, 0, "", 1, v_quiet);
    v_message = "This is the list and their versions.";
    screen_message (screen_size, 0, v_message, 1, v_quiet);
 ...
 // ****** R ******
    ...
    if (outR == "") {cout << "You need R to continue. Please install it and run kir-mapper setup again." << endl; return;}
    
    string Rfile = "kir-mapper_setup.R";
    ofstream myfileR;
    myfileR.open (Rfile);
    myfileR << "options(warn=-1)" << endl;
	myfileR << "list.of.packages <- c('ggplot2','plotly','htmlwidgets','stringr')" << endl;
    ...
    myfileR << "install.packages(new.packages)" << endl;
    myfileR << "}" << endl;
...
// ********** SAMTOOLS  ************
    ...
    cout << "Version lower than 1.18 won't work." << endl;
