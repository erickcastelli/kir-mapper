...
*/
    ...
    if (v_exome == 1)
    {
        v_command = "mkdir " + v_output + "/ncopy/capture_bias/";
        v_system_out = GetStdoutFromCommand(v_command);
    }
    ...
    debug_message("Copying threholds when it doesn't exist - start");
   ...
   debug_message("Copying threholds when it doesn't exist - end");
...
/*
            ...
            map <string,int> gene_sizes;
            map <pair<string,string>,float> depth_points_results;
            for (auto item : targetsdb)
            {
                string gene = item.first;
                string bedfile = v_db + "/genotype/bed/" + gene + ".CDS.bed";
                if (! fileExists(bedfile.c_str())) {continue;}
                ...
                for( std::string line; getline( bed, line ); )
                {
                    ...
                    vector <string> data;
                    boost::split(data,line,boost::is_any_of("\t"));
                    int dif =  stoi(data[2]) - + stoi(data[1]);
                    gene_sizes[gene] = gene_sizes[gene] + dif;
                }
                bed.close();
            }
            ...
            ThreadPool pool_cov_points(stoi(v_threads));
            std::vector< std::future<int> > results_cov_points;
            int loop = 1;
            int progress = 0;
            for (auto file : filedb)
            {
                    ...
                    results_cov_points.emplace_back(
                        pool_cov_points.enqueue([loop,file,filedb,sample,targetsdb,mincov,number_of_data_points,mindepthratio,&depth_points_results,&hetcounts,&progress,&gene_sizes]
                        {

                            for (auto item : targetsdb)
                            {
                                string gene = item.first;
                                string bedfile = v_db + "/genotype/bed/" + gene + ".full.bed";
                                if (! fileExists(bedfile.c_str())) {continue;}

                                string v_cmd = v_samtools + " view -q 1 -ML " + bedfile + " " + file.second;
                                string sam = GetStdoutFromCommand(v_cmd);
                                vector <string> data;
                                ...
                                {
                                    if (read == "") {continue;}
                                    if (read.substr(0,1) == "#"){continue;}
                                    vector <string> line;
                                    boost::split(line,read,boost::is_any_of("\t"));
                                    if (stoi(line[1]) > 200) {continue;}
                                    reads[line[0]]++;
                                }
                                mtx_ncopy.lock();
                                pair <string,string> key = make_pair(gene,sample);
                                //float adj = float(reads.size()) / gene_sizes[gene];
                                depth_points_results[key] = reads.size();
                                mtx_ncopy.unlock();


                                bedfile = v_db + "/genotype/bed/" + gene + ".CDS.bed";
                                v_cmd = v_samtools + " mpileup -l " + bedfile + " --ff SECONDARY,DUP -q 1 " + file.second;
                            ...
                            progress++;
                            mtx_ncopy.lock();
                            float pb = (float(progress) / float(filedb.size()))*100;
                            v_message = " > Processing ... " + to_string(pb) + "%";
                            screen_message (screen_size, 0, v_message, 0, v_quiet);
            ...
            for(auto && result: results_cov_points){result.get();} // waiting for all threads
            v_message = " > Processing ... done";
            screen_message (screen_size, 0, v_message, 1, v_quiet);


            string v_out_depth_points = v_output + "/teste.txt";
            ofstream DEPTHPOINTS;
            DEPTHPOINTS.open (v_out_depth_points.c_str());
            DEPTHPOINTS << "SAMPLE\tGENE\tKIR3DL3\tHLA-G\tHLA-E\tAVERAGE\tHET_POSITIVE\tHET_COUNT" << endl;
            ...
            for (auto file : filedb)
            {
                    ...
                    string sample = file.first;
                            ...
                            for (auto item : targetsdb)
                            {
                                ...
                                DEPTHPOINTS << sample << "\t" << gene;

                                pair <string,string> key = make_pair(gene,sample);
                                float test = 0;

                                if (depth_points_results.find(key) != depth_points_results.end()) {test = depth_points_results[key];}
       
                                float hets = 0;
                                if (hetcounts.find(key) != hetcounts.end()) {hets = hetcounts[key];}

                                
                                pair <string,string> key3DL3 = make_pair("KIR3DL3",sample);
                                float ref3DL3 = 0;
                                if (depth_points_results.find(key3DL3) != depth_points_results.end()) {ref3DL3 = depth_points_results[key3DL3];}


                                pair <string,string> keyG = make_pair("HLA-G",sample);
                                float refG = 0;
                                if (depth_points_results.find(keyG) != depth_points_results.end()) {refG = depth_points_results[keyG];}

                                pair <string,string> keyE = make_pair("HLA-E",sample);
                                float refE = 0;
                                if (depth_points_results.find(keyE) != depth_points_results.end()) {refE = depth_points_results[keyE];}

                                if (((ref3DL3 == 0) || (refG == 0)) || (refE == 0)){
                                    DEPTHPOINTS << "\tNA\tNA\tNA\tNA\tNA\tNA" << endl;
                                }


                                float ratio3DL3 = test / ref3DL3;
                                float ratioG = test / refG;
                                float ratioE = test / refE;
                                float aver = (ratio3DL3 + ratioG + ratioE) / 3;

                                DEPTHPOINTS << "\t" << ratio3DL3 << "\t" << ratioG << "\t" << ratioE << "\t" << aver;


                                if (hets > 0) {DEPTHPOINTS << "\t1";}
                                if (hets == 0) {DEPTHPOINTS << "\t0";}
                                DEPTHPOINTS << "\t" << hets;
...
map <pair<string,string>,string> depth_values_list;
  ...
  {
        debug_message("Evaluating depth of coverage - start");
        for (auto item : targetsdb)
        {
            string gene = item.first;
            
            v_message = " > Processing " + gene + " for coverage ...";
            screen_message (screen_size, 0, v_message, 1, v_quiet);
            ...
            for (auto file : filedb)
            {
                ...
                pool.enqueue([loop,file,gene,regions,mincov,mindepthratio,&hetcounts,&copyresults,&depth_values_list]
                {
                    ...
                    if (v_exome == 1) {
                        ...
                        depth = median(values);
/*
                        int min = quantile(0.25, values);
                        int max = quantile(0.75, values);
                        ...
                        {
                            //if ((item >= min) && (item <= max)){sum = sum = item; count++;}
                            sum = sum = item; count++;
                        }


                        depth = float(sum) / float(count);
                        */
                    }
        ...
        debug_message("Evaluating depth of coverage - end");
    ...
    for (auto file : filedb)
    {
        ...
        if (ref < mincovref) {
            failed_samples[sample] = 1; 
                v_message = " > warning: sample " + sample + " failed due to low coverage at the reference (" + to_string(ref) + ")";
                screen_message (screen_size, 0, v_message, 1, v_quiet);
        }
    }
        ...
        for (auto item : targetsdb)
        {
            ...
            if (failed_samples.find(sample) == failed_samples.end())
            {
                ...
                if (v_exome == 1) {
                    ...
                    depthvalue = float(sum) / float(count);
                    */
                }
